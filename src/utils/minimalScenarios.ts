...
/**
 * Simplified version for UI - gets key insights
 */
export function getTop2Insights(
  conf: ConferenceData,
  targetTeam: string
): {
  canFinishTop2: boolean;
  mustWinGames: Game[];
  helpfulWins: Array<{ game: Game; impact: number }>;
  probability: number;
} {
  const result = findMinimalTop2Scenarios(conf, targetTeam);

  if (!result.canFinishTop2) {
    return {
      canFinishTop2: false,
      mustWinGames: [],
      helpfulWins: [],
  ...
  // Find games where target team must win (appear in ALL scenarios)
  const gameWinCounts = new Map<string, number>();
  const gameMap = new Map<string, Game>();

  result.allConditions.forEach((cond) => {
    cond.gameOutcomes.forEach((go) => {
      if (go.mustWin === targetTeam) {
        const key = go.game.id;
        gameWinCounts.set(key, (gameWinCounts.get(key) || 0) + 1);
        gameMap.set(key, go.game);
      }
    });
  });

  const totalScenarios = result.allConditions.length;
  const mustWinGames: Game[] = [];
  const helpfulWins: Array<{ game: Game; impact: number }> = [];

  gameWinCounts.forEach((count, gameId) => {
    const game = gameMap.get(gameId)!;
    const impact = count / totalScenarios;

    if (impact === 1.0) {
      mustWinGames.push(game);
    } else if (impact > 0.5) {
      helpfulWins.push({ game, impact });
    }
  });

  // Sort helpful wins by impact
  helpfulWins.sort((a, b) => b.impact - a.impact);

  // Calculate overall probability
  const probability =
    result.allConditions.length /
    Math.pow(2, conf.games.filter((g) => !g.played).length);
